...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 5 year ago and filter to 30-35 months range."""
    try:
        # Fetch the last 5 year of data and filter to 30-35 months ago
        end_date = datetime.now() - timedelta(days=30 * 30)
        start_date = datetime.now() - timedelta(days=30 * 35)
        ...
        # Filter to keep only data between 30 and 35 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 30 to 35 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
