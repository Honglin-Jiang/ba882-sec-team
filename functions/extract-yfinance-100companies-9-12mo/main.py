...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 1 year ago and filter to 9-12 months range."""
    try:
        # Fetch the last 1 year of data and filter to 9-12 months ago
        end_date = datetime.now() - timedelta(days=30 * 9)
        ...
        # Filter to keep only data between 9 and 12 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 9 to 12 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
