...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 5 year ago and filter to 35-40 months range."""
    try:
        # Fetch the last 5 year of data and filter to 35-40 months ago
        end_date = datetime.now() - timedelta(days=30 * 35)
        start_date = datetime.now() - timedelta(days=30 * 40)
        ...
        # Filter to keep only data between 35 and 40 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 35 to 40 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
