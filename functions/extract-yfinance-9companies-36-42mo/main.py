...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 5 year ago and filter to 36-42 months range."""
    try:
        # Fetch the last 5 year of data and filter to 36-42 months ago
        end_date = datetime.now() - timedelta(days=30 * 36)
        start_date = datetime.now() - timedelta(days=30 * 42)
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance 9 companies stock data for 36 to 42 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
