...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 2 year ago and filter to 12-18 months range."""
    try:
        # Fetch the last 1 year of data and filter to 12-18 months ago
        end_date = datetime.now() - timedelta(days=30 * 12)
        start_date = datetime.now() - timedelta(days=30 * 18)
        
        # Fetch the last 1 year of data
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance 9 companies stock data for 12 to 18 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
