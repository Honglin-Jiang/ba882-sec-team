...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 5 year ago and filter to 24-30 months range."""
    try:
        # Fetch the last 5 year of data and filter to 24-30 months ago
        end_date = datetime.now() - timedelta(days=30 * 24)
        start_date = datetime.now() - timedelta(days=30 * 30)
        ...
        # Filter to keep only data between 24 and 30 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 24 to 30 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
