...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 2 year ago and filter to 12-15 months range."""
    try:
        # Fetch the last 2 year of data and filter 12-15 months ago
        end_date = datetime.now() - timedelta(days=30 * 12)
        start_date = datetime.now() - timedelta(days=30 * 15)
        ...
        # Filter to keep only data between 12 and 15 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 12 to 15 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
