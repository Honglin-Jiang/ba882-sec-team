...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 2 year ago and filter to 15-18 months range."""
    try:
        # Fetch the last 2 year of data and filter to 15-18 months ago
        end_date = datetime.now() - timedelta(days=30 * 15)
        start_date = datetime.now() - timedelta(days=30 * 18)
        ...
        # Filter to keep only data between 15 and 18 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 15 to 18 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
