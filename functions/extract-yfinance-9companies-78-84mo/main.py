...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 10 year ago and filter to 78-84 months range."""
    try:
        # Fetch the last 10 year of data and filter to 78-84 months ago
        end_date = datetime.now() - timedelta(days=30 * 78)
        start_date = datetime.now() - timedelta(days=30 * 84)
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance 9 companies stock data for 78 to 84 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
