...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 5 year ago and filter to 42-48 months range."""
    try:
        # Fetch the last 5 year of data and filter to 42-48 months ago
        end_date = datetime.now() - timedelta(days=30 * 42)
        start_date = datetime.now() - timedelta(days=30 * 48)
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance 9 companies stock data for 42 to 48 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
