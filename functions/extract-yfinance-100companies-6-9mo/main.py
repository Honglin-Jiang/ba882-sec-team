...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 1 year ago and filter to 6-9 months range."""
    try:
        # Fetch the last 1 year of data and filter to 6-9 months ago
        end_date = datetime.now() - timedelta(days=30 * 6)
        start_date = datetime.now() - timedelta(days=30 * 9)
        ...
        # Filter to keep only data between 6 and 9 months ago
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance data for 6 to 9 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
