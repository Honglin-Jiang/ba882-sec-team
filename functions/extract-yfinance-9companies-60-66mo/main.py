...
def get_yfinance_data(ticker):
    """Fetch stock data for the given ticker using yfinance for 10 year ago and filter to 60-66 months range."""
    try:
        # Fetch the last 10 year of data and filter to 60-66 months ago
        end_date = datetime.now() - timedelta(days=30 * 60)
        start_date = datetime.now() - timedelta(days=30 * 66)
...
def task(request):
    ...
    try:
        ...
        return {"message": "YFinance 9 companies stock data for 60 to 66 months ago successfully inserted into MotherDuck"}, 200
    except Exception as e:
